...
export default function LoadingScreen({
  ...
  const chartPath = useMemo(
    () => generateChartPath(600, 120, 30),
    // Regenerate the chart path each cycle in endless mode for variety
  ...
  useEffect(() => {
    ...
    if (endless) {
      // Loop forever: restart the cycle instead of exiting
      timers.push(setTimeout(() => setCycle((c) => c + 1), 2600));
    } else {
  ...
  return (
    <div
      className={`ld-screen${phase === 3 ? " ld-exit" : ""}${isLight ? " ld-light" : ""}${endless ? " ld-embedded" : ""}`}
    >
      ...
      <div className="ld-inner">
        ...
        <div
          key={`chart-${cycle}`}
          className={`ld-chart ${phase >= 1 ? "ld-chart-visible" : ""}`}
        >
